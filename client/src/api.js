...
export const updateSchemeProgress = async (savedSchemeId, progressData) => {
    const response = await fetch(`${API_URL}/saved-schemes/${savedSchemeId}/progress`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
export const compareSchemesAPI = async (schemeA, schemeB, language) => {
    const res = await fetch(`${API_URL}/compare-schemes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ schemeA, schemeB, language })
    });
    if (!res.ok) throw new Error('Failed to compare schemes');
    return res.json();
};

export const translateSchemeAPI = async (scheme, language) => {
    const res = await fetch(`${API_URL}/translate-scheme`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scheme, language })
    });
    if (!res.ok) throw new Error('Failed to translate scheme');
    return res.json();
};

export const submitVerifiedLink = async (schemeName, url) => {
    const response = await fetch(`${API_URL}/verified-links`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ schemeName, url }),
    });
    if (!response.ok) throw new Error("Failed to submit verified link");
    return await response.json();
};
