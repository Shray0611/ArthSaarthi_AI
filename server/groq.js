...
async function compareSchemes(schemeA, schemeB, language = 'en') {
  const langMap = { en: 'English', hi: 'Hindi', bn: 'Bengali', te: 'Telugu', mr: 'Marathi', ta: 'Tamil' };
  const langName = langMap[language] || 'English';

  const prompt = `
    You are an expert government scheme advisor. 
    A user is trying to decide between these two schemes they have saved.
    
    Scheme A:
    ${JSON.stringify(schemeA, null, 2)}
    
    Scheme B:
    ${JSON.stringify(schemeB, null, 2)}
    
    Task:
    Analyze and compare both schemes to help the user decide.
    Output STRICT JSON in the following format. 
    IMPORTANT: Do NOT use the generic terms "Scheme A" or "Scheme B" in your summary, differences, or recommendation. You MUST use their actual official names (e.g., "${schemeA.name || 'First Scheme'}" and "${schemeB.name || 'Second Scheme'}").
    {
      "winner": "The exact actual name of the winning scheme, or 'Both'",
      "summary": "Brief 2-3 sentence overview of the difference, using actual scheme names.",
      "schemeA_Pros": ["Pro 1", "Pro 2"],
      "schemeB_Pros": ["Pro 1", "Pro 2"],
      "overlapDocuments": ["Aadhaar Card", "Bank Passbook"],
      "keyDifferences": ["Difference 1", "Difference 2"],
      "recommendation": "Final verdict on who should choose what, using actual scheme names."
    }
    
    Rule: Output everything (except JSON keys) strictly in ${langName} language.
  `;
...
async function translateScheme(schemeDetails, language) {
  const langMap = { en: 'English', hi: 'Hindi', bn: 'Bengali', te: 'Telugu', mr: 'Marathi', ta: 'Tamil' };
  const langName = langMap[language] || 'English';

  const prompt = `
    You are an expert government scheme analyst and translator.
    You will receive a JSON object representing a government scheme.
    Your task is to process the text fields into the ${langName} language, while keeping the JSON structure and keys EXACTLY the same.
    
    CRITICAL INSTRUCTION FOR DESCRIPTION:
    Instead of just translating the short 'description' field, you MUST expand it into a very detailed, comprehensive 10-15 line description explaining the scheme's core purpose, background, target audience, and key benefits in depth. Make sure it is thorough, professional, and informative.

    Fields to process:
    - name (translate to ${langName})
    - description (EXPAND to 10-15 lines and translate to ${langName})
    - eligibilitySummary (array of strings, translate)
    - requiredDocuments (array of strings, translate)
    - applicationSteps (array of strings, translate)
    - benefits (array of strings, translate)
  ...
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        { role: "system", content: "You are a perfect JSON generator. You output ONLY valid JSON." },
        { role: "user", content: prompt }
      ],
      model: "llama-3.1-8b-instant",
      temperature: 0.2,
    ...
    console.error("Error in translateScheme:", error);
    throw error;
  }
}

module.exports = { recommendSchemes, chatWithScheme, searchSchemes, compareSchemes, translateScheme };
