const { recommendSchemes, chatWithScheme, searchSchemes, compareSchemes, translateScheme } = require('../groq');
const Analytics = require('../models/Analytics');
const History = require('../models/History');
const SavedScheme = require('../models/SavedScheme');
const VerifiedLink = require('../models/VerifiedLink');
...
/**
 * updateProgress
 * Updates the progress checklist of a saved scheme.
 */
exports.updateProgress = async (req, res) => {
    try {
        ...
        const updated = await SavedScheme.findByIdAndUpdate(
            id,
            { $set: { progress } },
            { new: true }
        );
 ...
 * Handles comparison of two schemes.
 */
exports.compareSchemesController = async (req, res) => {
    try {
        const { schemeA, schemeB, language } = req.body;
        if (!schemeA || !schemeB) {
            return res.status(400).json({ error: "Both schemeA and schemeB are required." });
        }
        const comparison = await compareSchemes(schemeA, schemeB, language || 'en');
        res.json(comparison);
    } catch (error) {
...
/**
 * submitVerifiedLink
 * Submits a new verified link from the community.
 */
exports.submitVerifiedLink = async (req, res) => {
    try {
        const { schemeName, url } = req.body;
        if (!schemeName || !url) {
            return res.status(400).json({ error: "schemeName and url are required." });
        }
        
        // Upsert the verified link
        const verifiedLink = await VerifiedLink.findOneAndUpdate(
            { schemeName },
            { verifiedUrl: url },
            { upsert: true, new: true }
        );
        
        res.json({ success: true, verifiedLink });
    } catch (error) {
        console.error("Submit link error:", error);
        res.status(500).json({ error: "Failed to submit verified link" });
    }
};

/**
 * translateSchemeController
 * Handles on-demand translation of a scheme
 */
exports.translateSchemeController = async (req, res) => {
    try {
        const { scheme, language } = req.body;
        if (!scheme || !language) {
            return res.status(400).json({ error: "scheme and language are required." });
        }
        const translated = await translateScheme(scheme, language);
        res.json({ translatedScheme: translated });
    } catch (error) {
